...
def run():
    ...
    while iters < training_iters:
        ...
        if buffer.trajs_in_buffer >= batch_size:
            ...
            state_for_mask = batch_state.reshape(batch_size, max_ep_length, -1, 2)[:,:,0:4]
            q_mask = th.ones_like(vdn_qs)
            q_mask[:,:,:,1] = 0
            q_mask[:,:,:,2] = 0
            for i in range(batch_size):
                for j in range(max_ep_length):
                    for k in range(n_agents):
                        if state_for_mask[i,j,k][0] == 0:
                            q_mask[i,j,k,0] = 0
                        if state_for_mask[i,j,k][1] == world_size-1:
                            q_mask[i,j,k,3] = 0
            ...
            if rl_manner == "sarsa":
                ...
                curr_max_qs = vdn_qs.max(dim=-1)[0]
                curr_mixed_maxq = curr_max_qs.sum(dim=-1).unsqueeze(-1)
