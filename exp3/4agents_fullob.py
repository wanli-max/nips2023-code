...
class Env():
    def __init__(self, world_size, punish):
        self.world_size = world_size
        self.n_agents = 4
        self.world = np.zeros((self.world_size,self.world_size))
        # assign landmark
        self.tars = np.array([0,1,0,1])
        self.reset()
        self.punish = punish
        self.ret = 0
    ...
    def step(self,ac,render=False):
        ...
        if self.world[0,0] == 1 and self.world[0,-1] == 2:
            terminated = True
        if render:
            time.sleep(0.1)
            self.render()
    ...
    def render(self):
        ...
        for i in range(self.n_agents):
            if (self.poses[i][0]==0 and self.poses[i][1]==0) or (self.poses[i][0]==0 and self.poses[i][1]==self.world_size-1):
                pass
            else:
...
class Qnet_CRC(nn.Module):
    def __init__(self,n_agents, world_size):
        ...
        self.state_shape = n_agents * 2                      # poses
        self.world_size = world_size
...
class ReplayBuffer:
    def __init__(self, max_ep_length, buffer_length, n_agents):
        ...
        self.dict = {"state": th.zeros(self.buffer_length, max_ep_length, 2* self.n_agents),
                    "obs": th.zeros(self.buffer_length, max_ep_length, self.n_agents, 2),
...
def run():
    ...
    if decomposable:
        punish = 0
        env = Env(world_size,punish=punish)
    else:
        punish = 0
        env = Env_ind(world_size,punish=punish)
    ...
    while iters < training_iters:
        ...
        while t < max_ep_length and (not terminated):
            obs = th.from_numpy(state).unsqueeze(0).repeat(4,1).float()
            if iters % 100 == 0 and iters > 0:    # test mode
                ...
                if evaluate_algo == "qcrc":
                    ac, acs = qnet_true.select_acs(state, 1)
                else:
                    ac,acs = q_vdn.select_acs(state, obs, 1)
            ...
            episode_data["obs"] = th.cat((episode_data["obs"], th.tensor([state]).reshape(n_agents,2).unsqueeze(0).float()), dim=0)
        ...
        if buffer.trajs_in_buffer >= batch_size:
            ...
            batch_obs = batch_state.unsqueeze(0).repeat(n_agents,1,1,1)
